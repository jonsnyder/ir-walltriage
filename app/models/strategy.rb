class Strategy < ActiveRecord::Base
  ...
  def compute_stats
    ...
    AccessToken.where(:use_for_stats => true).each do |user|
      ...
      stat_values.create( :stat => Stat.kappa, :access_token => user, :value => kappa.score)
      stat_values.create( :stat => Stat.precision, :access_token => user, :value => kappa.precision)
      stat_values.create( :stat => Stat.recall, :access_token => user, :value => kappa.recall)
    end
